...
pub fn range_query<K: Ord + Clone, V: Clone>(map: &BTreeMap<K, V>, from: &K, to: &K) -> Vec<(K, V)> {
    map.range(from..=to).map(|(k, v)| (k.clone(), v.clone())).collect()
}
