...
fn main() {
    ...
    let doubled = map_idiomatic(&numbers, |x| x * 2);
    println!("Original: {:?}", numbers);
    println!("Doubled (idiomatic): {:?}", doubled);
    let doubled_rec = map_recursive(&numbers, &|x| x * 2);
    println!("Doubled (recursive): {:?}", doubled_rec);
}

/* Output:
   Original: [1, 2, 3, 4, 5]
   Doubled (idiomatic): [2, 4, 6, 8, 10]
   Doubled (recursive): [2, 4, 6, 8, 10]
*/