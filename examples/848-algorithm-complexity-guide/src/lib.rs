...
pub fn bubble_sort(arr: &mut [i32]) {
    let n = arr.len();
    for i in 0..n { for j in 0..n-i-1 { if arr[j] > arr[j+1] { arr.swap(j, j+1); } } }
}
...
mod tests {
    ...
    #[test] fn test_constant() { assert_eq!(constant_time_example(&[1, 2, 3]), Some(1)); }
    #[test] fn test_binary() { assert_eq!(binary_search(&[1, 2, 3, 4, 5], 3), Some(2)); }
    #[test] fn test_linear() { assert_eq!(linear_search(&[1, 2, 3, 4, 5], 3), Some(2)); }
    #[test] fn test_merge() { assert_eq!(merge_sort(vec![3, 1, 4, 1, 5, 9]), vec![1, 1, 3, 4, 5, 9]); }
}
