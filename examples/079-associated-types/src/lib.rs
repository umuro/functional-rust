...
struct IntStack { elements: Vec<i32> }

impl Container for IntStack {
    type Item = i32;
    fn new() -> Self { IntStack { elements: Vec::new() } }
...
struct StringQueue { elements: Vec<String> }

impl Container for StringQueue {
    type Item = String;
    fn new() -> Self { StringQueue { elements: Vec::new() } }
    fn push(&mut self, item: String) { self.elements.push(item); }
    fn pop(&mut self) -> Option<String> {
        if self.elements.is_empty() { None } else { Some(self.elements.remove(0)) }
    }
    fn is_empty(&self) -> bool { self.elements.is_empty() }
}
...
struct RangeIter { current: i32, stop: i32 }

impl Iterator for RangeIter {
    type Item = i32;
    fn next(&mut self) -> Option<Self::Item> {
        if self.current >= self.stop { None }
...
mod tests {
    ...
    #[test]
    fn test_range_iter() {
        let items: Vec<i32> = (RangeIter { current: 0, stop: 5 }).collect();
