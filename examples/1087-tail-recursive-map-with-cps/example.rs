...
fn main() {
    ...
    println!("map_idiomatic([1..5], x*2) = {:?}", map_idiomatic(nums, |x| x * 2));
    println!("map_tr([1..5], x*2)        = {:?}", map_tr(nums, |x| x * 2));
    println!("map_cps([1..5], x*2)       = {:?}", map_cps(nums, |x| x * 2));
    println!("map_naive([1..5], x*2)     = {:?}", map_naive(nums, &|x| x * 2));

    println!();
    println!("map_idiomatic([], x*2) = {:?}", map_idiomatic(&[] as &[i32], |x| x * 2));
    println!("map_tr([], x*2)        = {:?}", map_tr(&[] as &[i32], |x| x * 2));

    println!();
    let words = &["hello", "world", "rust"];
    println!("map lengths: {:?}", map_idiomatic(words, |s| s.len()));
}

/* Output:
   map_idiomatic([1..5], x*2) = [2, 4, 6, 8, 10]
   map_tr([1..5], x*2)        = [2, 4, 6, 8, 10]
   map_cps([1..5], x*2)       = [2, 4, 6, 8, 10]
   map_naive([1..5], x*2)     = [2, 4, 6, 8, 10]
