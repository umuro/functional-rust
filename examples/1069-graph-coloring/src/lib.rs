...
fn graph_coloring(adj: &[Vec<i32>], num_colors: usize) -> Option<Vec<usize>> {
    ...
    if solve(0, adj, &mut colors, num_colors) { Some(colors) } else { None }
}
...
fn greedy_coloring(adj: &[Vec<usize>]) -> Vec<usize> {
    ...
    let mut colors = vec![0usize; n];
    for node in 0..n {
        let used: std::collections::HashSet<usize> = adj[node].iter()
...
mod tests {
    ...
    fn test_graph_coloring_possible() {
        let adj = vec![vec![0,1,1,1], vec![1,0,1,0], vec![1,1,0,1], vec![1,0,1,0]];
        let colors = graph_coloring(&adj, 3).unwrap();
        assert_eq!(colors.len(), 4);
        // Verify no adjacent nodes share color
        for i in 0..4 {
            for j in 0..4 {
                if adj[i][j] == 1 { assert_ne!(colors[i], colors[j]); }
            }
        }
    }
