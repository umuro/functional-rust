...
fn main() {
    ...
    println!("Sum: {}, Product: {}, Max: {}", sum_iter, product_iter, max_iter);

    println!("\n=== Recursive fold_left ===");
    println!("Sum: {}, Product: {}, Max: {}", sum_rec, product_rec, max_rec);
    ...
    println!("Empty list - sum: {}, product: {}, max: {}", sum(&empty), product(&empty), max_value(&empty));

    let single = vec![42];
    println!("Single element [42] - sum: {}, product: {}, max: {}", sum(&single), product(&single), max_value(&single));

    let negative = vec![-5, 10, -3];
    println!("Negative values [-5, 10, -3] - sum: {}, product: {}, max: {}", sum(&negative), product(&negative), max_value(&negative));
}
