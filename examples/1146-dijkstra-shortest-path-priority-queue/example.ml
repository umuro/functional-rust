(* Dijkstra's shortest path using OCaml's functional data structures.
   Priority queue: Set.Make (ordered by (distance, node) — min element first)
   Distance map:   Map.Make(String) *)
...
(* Idiomatic OCaml: recursive loop over a functional priority queue *)
      ...
      let dist, pq =
        List.fold_left
          (fun (dist, pq) (v, w) ->
            ...
            if alt < current then (SMap.add v alt dist, PQ.add (alt, v) pq)
            else (dist, pq))
          (dist, pq) neighbors
      in
      go pq dist
...
let () =
  ...
  assert (SMap.find "c" dist = 3); (* a->b->c = 1+2, not a->c = 4 *)
