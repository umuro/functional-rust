📖 **[View on hightechmind.io →](https://hightechmind.io/rust/474-string-formatting)**

---

# 474-string-formatting — String Formatting

See [hightechmind.io/rust/474-string-formatting](https://hightechmind.io/rust/474-string-formatting) for the full explanation, OCaml comparison, and tests.

## Quick Start

```bash
cargo test
```

## Files

- `example.ml` — OCaml implementation
- `example.rs` — Rust implementation  
- `src/lib.rs` — Rust library code
- `COMPARISON.md` — Key differences between OCaml and Rust approaches
