...
impl<T: Ord + Clone> RBTree<T> {
    ...
    /// Restores red-black invariants after insertion (Okasaki's 4-case balance).
    ...
    /// Inserts a value, returning a new balanced tree (persistent).
    ...
    /// Checks membership via recursive BST descent.
    ...
    /// In-order traversal producing a sorted vector.
...
fn main() {
    // Mirrors the OCaml example: List.fold_left insert E [5;3;7;1;4;6;8;2;9]
    ...
    // Demonstrate persistence: inserting returns a new tree
    let tree2 = tree.insert(10);
    println!("After insert(10): {:?}", tree2.to_sorted_vec());
    println!("Original still: {:?}", tree.to_sorted_vec());
}
...
/* Output:
   ...
   Original still: [1, 2, 3, 4, 5, 6, 7, 8, 9]
*/
