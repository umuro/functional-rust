...
(* Rosetta Code Averages/Mean time of day implementation in OCaml *)
...
let () =
  ...
  assert (string_of_time (mean_time (List.map parse_time times)) = string_of_time (mean_time (List.map parse_time times)));
  let single = [parse_time "12:30:00"] in
  assert (string_of_time (mean_time single) = "12:30:0");
  print_endline "ok"
