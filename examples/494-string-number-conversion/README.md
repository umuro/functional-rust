📖 **[View on hightechmind.io →](https://hightechmind.io/rust/494-string-number-conversion)**

---

# 494-string-number-conversion — String Number Conversion

See [hightechmind.io/rust/494-string-number-conversion](https://hightechmind.io/rust/494-string-number-conversion) for the full explanation, OCaml comparison, and tests.

## Quick Start

```bash
cargo test
```

## Files

- `example.ml` — OCaml implementation
- `example.rs` — Rust implementation  
- `src/lib.rs` — Rust library code
- `COMPARISON.md` — Key differences between OCaml and Rust approaches
