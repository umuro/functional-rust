...
fn main() {
    let msg = "Testing, 1 2 3, testing.";
    let encoded = encode(msg);
    let decoded = decode(&encoded);

    println!("encode({:?}) = {:?}", msg, encoded);
    println!("decode({:?}) = {:?}", encoded, decoded);
    println!("encode_recursive({:?}) = {:?}", msg, encode_recursive(msg));
    println!("encode(\"Hello, World!\") = {:?}", encode("Hello, World!"));
    println!("decode(\"svool\") = {:?}", decode("svool"));
}
