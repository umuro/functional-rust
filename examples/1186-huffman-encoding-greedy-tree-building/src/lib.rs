...
/// Avoids repeated sorting — O(n log n) overall vs O(n² log n) for the sort-each-round approach.
pub fn build_tree_idiomatic(freqs: &[(char, u32)]) -> Option<HTree> {
    use std::collections::BinaryHeap;

    // Local wrapper: BinaryHeap needs Ord, but ordering HTree by structure makes no sense.
    // We order only by (freq, counter) — counter breaks ties deterministically (FIFO).
    struct Item {
        freq: u32,
        counter: usize,
        tree: HTree,
    ...
    impl Ord for Item {
        fn cmp(&self, other: &Self) -> std::cmp::Ordering {
            ...
            // Break ties by counter so insertion order is preserved.
            other
                .freq
                .cmp(&self.freq)
...
mod tests {
    ...
    #[test]
    fn test_codes_recursive_vs_idiomatic_same_charset() {
