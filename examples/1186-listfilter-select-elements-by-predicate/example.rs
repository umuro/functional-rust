...
fn main() {
    ...
    let evens_str: Vec<String> = evens.iter().map(|x| x.to_string()).collect();
    let odds_str: Vec<String> = odds.iter().map(|x| x.to_string()).collect();

    println!("Evens: {}", evens_str.join(", "));
    println!("Odds: {}", odds_str.join(", "));

    // Owned filter
    let data = vec![10, 15, 20, 25, 30];
    let big: Vec<i32> = filter_owned(data, |x| *x > 15);
    println!("Greater than 15: {:?}", big);

    // Recursive style
    let rec_evens = filter_recursive(&numbers, &|x: &i32| x % 2 == 0);
    println!("Evens (recursive): {:?}", rec_evens);
}
