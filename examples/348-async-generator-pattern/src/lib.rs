...
pub struct Generator<T> { items: Vec<T>, index: usize }
...
impl<T: Clone> Iterator for Generator<T> {
    type Item = T;
    fn next(&mut self) -> Option<Self::Item> { Generator::next(self) }
}
...
mod tests {
    ...
    #[test] fn generator_reset() {
        let mut g = Generator::new(vec![1, 2]);
        g.next(); g.next(); g.reset();
        assert_eq!(g.next(), Some(1));
    }
    #[test] fn as_iterator() {
