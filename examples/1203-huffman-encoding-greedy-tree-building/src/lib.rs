...
/// A Huffman tree — a leaf holds a symbol + frequency;
/// an internal node holds two subtrees + combined frequency.
#[derive(Debug, Clone)]
pub enum HTree {
    Leaf(char, u32),
...
// ---------------------------------------------------------------------------
// Solution 1: Idiomatic Rust — BinaryHeap (min-heap via Reverse ordering)
// ---------------------------------------------------------------------------

/// Newtype so `BinaryHeap` acts as a *min*-heap (lowest frequency first).
...
impl Ord for MinTree {
    fn cmp(&self, other: &Self) -> Ordering {
        // Flip ordering so the *smallest* frequency is at the top.
...
pub fn build_tree_idiomatic(freqs: &[(char, u32)]) -> Option<HTree> {
    ...
    while heap.len() > 1 {
        // Pop the two lowest-frequency trees.
...
// ---------------------------------------------------------------------------
// Solution 2: Functional/recursive — mirrors OCaml's List.sort + `go`
// ---------------------------------------------------------------------------

/// Build a Huffman tree by repeatedly sorting and merging, exactly like
/// the OCaml `go` inner function. Returns `None` for an empty input.
...
fn go(mut trees: Vec<HTree>) -> HTree {
    match trees.len() {
        0 => panic!("go: empty list — unreachable via public API"),
        1 => trees.remove(0),
        _ => {
            // Merge the two lowest-frequency trees (front of sorted vec).
...
// ---------------------------------------------------------------------------
// Code extraction — traverse the tree, accumulating "0"/"1" prefix bits
// ---------------------------------------------------------------------------

/// Returns all `(symbol, binary-code-string)` pairs for the given tree.
/// Mirrors OCaml's recursive `codes` function.
...
mod tests {
    ...
    #[test]
    fn test_codes_most_frequent_has_shortest_code() {
        // 'f' (freq 45) should have the shortest code.
        let tree = build_tree_recursive(FREQS).unwrap();
        let result = codes(&tree);
        let f_len = result
            .iter()
    ...
    fn test_codes_unique() {
        // Every symbol gets a distinct code.
        let tree = build_tree_recursive(FREQS).unwrap();
        let result = codes(&tree);
