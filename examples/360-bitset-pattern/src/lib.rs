...
impl BitSet64 {
    ...
    pub fn insert(&mut self, i: u32) { assert!(i < 64); self.0 |= 1u64 << i; }
    pub fn remove(&mut self, i: u32) { if i < 64 { self.0 &= !(1u64 << i); } }
    pub fn contains(&self, i: u32) -> bool { i < 64 && (self.0 >> i) & 1 == 1 }
    ...
    pub fn to_vec(&self) -> Vec<u32> { (0..64).filter(|&i| self.contains(i)).collect() }
