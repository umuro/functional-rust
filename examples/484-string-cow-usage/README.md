📖 **[View on hightechmind.io →](https://hightechmind.io/rust/484-string-cow-usage)**

---

# 484-string-cow-usage — String Cow Usage

See [hightechmind.io/rust/484-string-cow-usage](https://hightechmind.io/rust/484-string-cow-usage) for the full explanation, OCaml comparison, and tests.

## Quick Start

```bash
cargo test
```

## Files

- `example.ml` — OCaml implementation
- `example.rs` — Rust implementation  
- `src/lib.rs` — Rust library code
- `COMPARISON.md` — Key differences between OCaml and Rust approaches
