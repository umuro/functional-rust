...
pub struct MultiMap<K, V> { inner: HashMap<K, Vec<V>> }
...
impl<K: Eq + std::hash::Hash, V> MultiMap<K, V> {
    ...
    pub fn get(&self, key: &K) -> Option<&Vec<V>> { self.inner.get(key) }
