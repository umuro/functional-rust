...
fn range(lo: i64, hi: i64) -> FixList {
    ana(&|s: (i64, i64)| {
        if s.0 > s.1 { ListF::NilF } else { ListF::ConsF(s.0, (s.0 + 1, s.1)) }
    }, (lo, hi))
}

// Approach 2: Countdown
fn countdown(n: i64) -> FixList {
    ana(&|s| if s <= 0 { ListF::NilF } else { ListF::ConsF(s, s - 1) }, n)
}
...
fn balanced_tree(depth: u32) -> FixTree {
    ana_tree(&|s: (u32, i64)| {
        if s.0 == 0 { TreeF::LeafF(s.1) }
        else { TreeF::BranchF((s.0 - 1, s.1), (s.0 - 1, s.1 + (1i64 << (s.0 - 1)))) }
    }, (depth, 1))
}
...
fn tree_to_vec(t: &FixTree) -> Vec<i64> {
    fn go(t: &FixTree) -> Vec<i64> {
        match t.0.as_ref() {
            TreeF::LeafF(n) => vec![*n],
            TreeF::BranchF(l, r) => { let mut v = go(l); v.extend(go(r)); v }
        }
    }
    go(t)
}
...
mod tests {
    ...
    #[test]
    fn test_range() { assert_eq!(to_vec(&range(1, 3)), vec![1, 2, 3]); }

    #[test]
    fn test_countdown() { assert_eq!(to_vec(&countdown(3)), vec![3, 2, 1]); }

    #[test]
    fn test_collatz_1() { assert_eq!(to_vec(&collatz(1)), vec![1]); }

    #[test]
    fn test_balanced_tree() { assert_eq!(tree_to_vec(&balanced_tree(1)), vec![1, 2]); }
}
