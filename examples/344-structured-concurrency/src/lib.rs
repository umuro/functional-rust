...
pub fn scoped_work<F, R>(work: F) -> R where F: FnOnce() -> R + Send, R: Send {
    thread::scope(|s| {
...
pub fn parallel_sum(nums: &[i32]) -> i32 {
    if nums.len() < 100 { return nums.iter().sum(); }
    let mid = nums.len() / 2;
...
mod tests {
    use super::*;
    #[test] fn scoped_returns_result() { assert_eq!(scoped_work(|| 42), 42); }
    #[test] fn parallel_sum_correct() {
