...
pub fn total_area_static<T: Shape>(shapes: &[T]) -> f64 { shapes.iter().map(|s| s.area()).sum() }

// Dynamic dispatch (vtable) - runtime
pub fn total_area_dynamic(shapes: &[Box<dyn Shape>]) -> f64 { shapes.iter().map(|s| s.area()).sum() }
...
mod tests {
    ...
    #[test] fn test_square_area() { let s = Square { side: 3.0 }; assert_eq!(s.area(), 9.0); }
    #[test] fn test_circle_area() { let c = Circle { r: 1.0 }; assert!((c.area() - std::f64::consts::PI).abs() < 0.001); }
}
