📖 **[View on hightechmind.io →](https://hightechmind.io/rust/482-string-unicode)**

---

# 482-string-unicode — String Unicode

See [hightechmind.io/rust/482-string-unicode](https://hightechmind.io/rust/482-string-unicode) for the full explanation, OCaml comparison, and tests.

## Quick Start

```bash
cargo test
```

## Files

- `example.ml` — OCaml implementation
- `example.rs` — Rust implementation  
- `src/lib.rs` — Rust library code
- `COMPARISON.md` — Key differences between OCaml and Rust approaches
