...
fn main() {
    // Idiomatic: mutate in place
    let src = [10, 20, 30, 40, 50];
    let mut dst = [0i32; 8];
    array_blit(&src, 1, &mut dst, 2, 3);
    println!("array_blit (in-place): {:?}", dst);

    // Functional: immutable, returns new Vec
    let dst2 = [0i32; 8];
    let result = array_blit_functional(&src, 1, &dst2, 2, 3);
    println!("array_blit_functional: {:?}", result);
}

/* Output:
   array_blit (in-place): [0, 0, 20, 30, 40, 0, 0, 0]
   array_blit_functional: [0, 0, 20, 30, 40, 0, 0, 0]
*/
