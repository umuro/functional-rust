(* 1055: Longest Common Subsequence — 2D DP + Backtracking *)

(* Approach 1: 2D DP for length only *)
let lcs_length s1 s2 =
  let a = Array.init (String.length s1) (String.get s1) in
  let b = Array.init (String.length s2) (String.get s2) in
  let m = Array.length a and n = Array.length b in
  let dp = Array.init (m + 1) (fun _ -> Array.make (n + 1) 0) in
  for i = 1 to m do
    for j = 1 to n do
      dp.(i).(j) <-
        if a.(i-1) = b.(j-1) then dp.(i-1).(j-1) + 1
        else max dp.(i-1).(j) dp.(i).(j-1)
    done
  done;
  dp.(m).(n)

(* Approach 2: DP + backtrack to reconstruct the LCS string *)
let lcs_string s1 s2 =
  let a = Array.init (String.length s1) (String.get s1) in
  let b = Array.init (String.length s2) (String.get s2) in
  let m = Array.length a and n = Array.length b in
  let dp = Array.init (m + 1) (fun _ -> Array.make (n + 1) 0) in
  for i = 1 to m do
    for j = 1 to n do
      dp.(i).(j) <-
        if a.(i-1) = b.(j-1) then dp.(i-1).(j-1) + 1
        else max dp.(i-1).(j) dp.(i).(j-1)
    done
  done;
  (* Backtrack *)
  let result = Buffer.create 16 in
  let i = ref m and j = ref n in
  while !i > 0 && !j > 0 do
    if a.(!i - 1) = b.(!j - 1) then begin
      Buffer.add_char result a.(!i - 1);
      decr i; decr j
    end else if dp.(!i - 1).(!j) > dp.(!i).(!j - 1) then
      decr i
    else
      decr j
  done;
  (* Buffer was built in reverse *)
  let s = Buffer.contents result in
  let len = String.length s in
  String.init len (fun i -> s.[len - 1 - i])

(* Approach 3: Recursive with memoization *)
let lcs_memo s1 s2 =
  let a = Array.init (String.length s1) (String.get s1) in
  let b = Array.init (String.length s2) (String.get s2) in
  let m = Array.length a and n = Array.length b in
  let cache = Hashtbl.create 64 in
  let rec solve i j =
    if i = 0 || j = 0 then 0
    else match Hashtbl.find_opt cache (i, j) with
    | Some v -> v
    | None ->
      let v =
        if a.(i-1) = b.(j-1) then solve (i-1) (j-1) + 1
        else max (solve (i-1) j) (solve i (j-1))
      in
      Hashtbl.add cache (i, j) v; v
  in
  solve m n

let () =
  assert (lcs_length "abcde" "ace" = 3);
  assert (lcs_length "abc"   "abc" = 3);
  assert (lcs_length "abc"   "def" = 0);

  assert (lcs_string "abcde"  "ace"     = "ace");
  assert (lcs_string "AGGTAB" "GXTXAYB" = "GTAB");

  assert (lcs_memo "abcde"  "ace"     = 3);
  assert (lcs_memo "AGGTAB" "GXTXAYB" = 4);

  Printf.printf "All LCS tests passed.\n"
