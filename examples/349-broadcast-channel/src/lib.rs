...
pub struct BroadcastReceiver<T> { rx: mpsc::Receiver<T> }
...
impl<T: Clone + Send + 'static> BroadcastSender<T> {
    ...
    pub fn send(&self, msg: T) {
        ...
        for sub in subs.iter() { let _ = sub.try_send(msg.clone()); }
    }
}
...
impl<T> BroadcastReceiver<T> {
    pub fn recv(&self) -> Option<T> { self.rx.recv().ok() }
...
mod tests {
    ...
    #[test] fn late_subscriber_misses() {
        let sender = BroadcastSender::new();
        sender.send(1);
